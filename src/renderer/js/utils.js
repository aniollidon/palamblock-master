...
export function minutstoDDHHMM(minuts) {
  ...
  let o = [];
  if (dd === 1) o.push(dd + " dia");
  else if (dd > 1) o.push(dd + " dies");
  if (hh === 1) o.push(hh + " hora");
  else if (hh > 1) o.push(hh + " hores");
  if (mm === 1) o.push(mm + " minut");
  else if (mm > 1) o.push(mm + " minuts");

  if (o.length === 1) return o[0];
  else return o.slice(0, -1).join(" ") + " i " + o.slice(-1);
}
...
export function getIntervalHorari(moment, sessions) {
  ...
  for (let hhmm in commonHorari) {
    ...
    if (minuts > momentM) {
      count_sessions++;
      if (count_sessions >= sessions) return commonHorari[hhmm];
    }
  }
...
export function isSuperUser() {
  ...
  try {
    const v =
      sessionStorage.getItem("pbk:super") ?? sessionStorage.getItem("super");
    isSuper = v === "1" || v === "true" || v === "yes" || v === "on";
  } catch (_) {}
  return isSuper;
}
