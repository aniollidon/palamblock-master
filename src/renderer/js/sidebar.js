...
function emitWhenReady(eventName, payload) {
  ...
  const onReady = () => {
    ...
    // petit delay per assegurar connexió estable
...
export function toogleSideBar(alumne, tipus = "web") {
  ...
  if (
    prevTipus !== tipus ||
    prevAlumne !== alumne ||
    historialSidebar.style.display.includes("none")
  ) {
    // Assegura que no tingui una classe que el forci a ocultar-se
    if (historialSidebar.classList.contains("d-none-force")) {
      historialSidebar.classList.remove("d-none-force");
    }
    initHistorialSidebar(alumne);
    // Reset tabs to  i ask for the new data
    const historialSidebarAllTab = document.getElementById(
      "historialSidebar-allTab"
    );
    historialSidebarAllTab.click();

    const historialSideBarTitle = document.getElementById(
      "historialSidebarTitle"
    );
    const historialSideBarContent = document.getElementById(
      "historialSidebarContent"
    );
    historialSideBarTitle.innerHTML = `Historial ${
...
export function moveHistorialSidebarToSearch(query) {
  const historialSidebarSearchTab = document.getElementById(
    "historialSidebar-searchTab"
  );
  const historialSidebarSearchInput = document.getElementById(
...
export function initHistorialSidebar(alumne) {
  ...
  const historialSidebarStatsTab = document.getElementById(
    "historialSidebar-statsTab"
  );
  const historialSidebarSortedTab = document.getElementById(
    "historialSidebar-sortedTab"
  );
  const historialSidebarSearchTab = document.getElementById(
    "historialSidebar-searchTab"
  );
  ...
  const historialGraphSidebarContent = document.getElementById(
    "historialGraphSidebarContent"
  );
  const historialSortedSidebarContent = document.getElementById(
    "historialSortedSidebarContent"
  );
  const historialSidebarSearchContent = document.getElementById(
    "historialSearchSidebarContent"
  );
  const historialSearchResults = document.getElementById(
  ...
  historialSidebarAllTab.onclick = () => {
    ...
    emitWhenReady("getHistorialWeb", { alumne: alumne });
  };
  ...
  historialSidebarStatsTab.onclick = () => {
    ...
    emitWhenReady("getEachBrowserLastUsage", { alumne: alumne, pastDays: 7 });
  };
  ...
  historialSidebarSortedTab.onclick = () => {
    ...
    emitWhenReady("getHistorialHostsSortedByUsage", { alumne: alumne });
  };
  ...
  historialSidebarSearchTab.onclick = () => {
    ...
    historialSearchInput.onchange = () => {
      ...
      if (search.length > 2) {
        ...
        emitWhenReady("getSearchHistorialWeb", {
          alumne: alumne,
          search: search,
        });
      }
...
export function drawHistorialWeb(alumne, historial, query) {
  ...
  if (query && hiddenAuxInfo.getAttribute("data-query") !== query) {
    ...
    if (
      historial.length === 0 &&
      hiddenAuxInfo.getAttribute("data-historial-length") === "0"
    ) {
  ...
  const historialLength =
    parseInt(hiddenAuxInfo.getAttribute("data-historial-length")) +
    historial.length;
  ...
  for (const webPage of historial) {
    ...
    const hora = data.toLocaleTimeString("ca-ES", {
      hour: "2-digit",
      minute: "2-digit",
    });
    ...
    if (prevhost && previd && prevhost === webPage.host && !newDay) {
      ...
      if (dHoraEnd !== hora) dHora.innerHTML = `${hora} - ${dHoraEnd}`;
      else dHora.innerHTML = hora;
      continue;
    }
    ...
    a.setAttribute(
      "class",
      "list-group-item list-group-item-action lh-tight py-1"
    ); //active
    if (webPage.pbAction === "block") a.classList.add("text-danger");
    const tooltip =
      "Obert a " +
      webPage.browser +
      (webPage.incognito ? " en mode incognit" : "") +
      `\n${webPage.title}` +
    ...
    const noicon =
      webPage.protocol && noprotocols.indexOf(webPage.protocol) !== -1;
    favicon.src =
      webPage.favicon && !noicon
        ? webPage.favicon
        : "images/undefined_favicon.png";

    favicon.onload = () => {
      if (favicon.naturalWidth === 0) {
        favicon.src = "images/undefined_favicon.png";
      }
    };
    favicon.onerror = () => {
      favicon.src = "images/undefined_favicon.png";
    ...
    a.onclick = (ev) => {
      ...
      const opcionMenuContextual = creaWebMenuJSON(
        alumne,
        undefined,
        webPage.pbAction === "block"
      );
  ...
  if (historial.length !== 0) {
    ...
    a.setAttribute(
      "class",
      "list-group-item list-group-item-action list-group-item-dark lh-tight"
    );
    ...
    a.onclick = () => {
      if (query)
        emitWhenReady("getSearchHistorialWeb", {
          alumne: alumne,
          search: query,
          offset: historialLength,
        });
      else
        emitWhenReady("getHistorialWeb", {
          alumne: alumne,
          offset: historialLength,
        });
      a.remove();
...
export function drawHistorialHostsSortedByUsage(alumne, sortedHistorial, days) {
  ...
  titleh7.setAttribute(
    "class",
    "bg-light border-top border-bottom date-historial-heading px-2"
  );
  ...
  buttonchangeHistorialDays.setAttribute(
    "class",
    "border-0 button-calendar-days"
  );
...
export function drawHistorialStats(alumne, lastUsage) {
  ...
  for (const browser in lastUsage) {
    const lastUsageDate = new Date(lastUsage[browser]).toLocaleDateString(
      "ca-ES",
      {
        year: "numeric",
        month: "2-digit",
    ...
    dTitile.setAttribute("class", "mb-1 nomesunalinia");
    const favicon = document.createElement("img");
    favicon.setAttribute("src", `images/${browser.toLowerCase()}.png`);
    favicon.setAttribute("class", "historial-favicon");
    dTitile.appendChild(favicon);
