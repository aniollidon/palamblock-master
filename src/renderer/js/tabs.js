((window, factory) => {
  ...
  class ChromeTabs {
    ...
    addTab(tabProperties, { animate = true, background = false } = {}) {
      ...
      else tabContentEl.classList.remove("chrome-tab-blocked");
    ...
    setTabClickEventListener(tabEl) {
      tabEl
        .querySelector(".chrome-tab-content")
        .addEventListener("click", (e) => {
          // Opcions del menu
...
// Dynamic resolver because views are swapped at runtime
function getMenuEl() {
...
const openMenu = (e, options, info) => {
  ...
  if (
    menuClickCoords.left === origin.left &&
    menuClickCoords.top === origin.top
  )
    return closeMenu(e);
  ...
  if (!menu) return false; // menu not present in this view yet
...
const toggleMenu = (command) => {
  const menu = getMenuEl();
  if (!menu) return; // no-op if not in DOM
...
// Re-hide menu when views are swapped
